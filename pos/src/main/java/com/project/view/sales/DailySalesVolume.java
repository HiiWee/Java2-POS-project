...
public class DailySalesVolume extends JPanel {
    ...
    public void paint(Graphics g){
        ...
        for(int i = 0; i < dateData.date.size(); i++){ g.drawString(dateData.date.get(i), valueLength-30+i*valueLength, 520); }    //그래프 x축에 날짜 넣기
        g.setColor(Color.RED);
        for(int i = 0; i < dateData.date.size(); i++){
            if(dateData.dateValue.get(i) > 0)
                g.fillRect(valueLength-rectLength/2+i*valueLength, 500-(4*dateData.dateValue.get(i)/((dateData.max/500+1)*5)), rectLength, 4*dateData.dateValue.get(i)/((dateData.max/500+1)*5)); //사각 그래프 그리기
        }
